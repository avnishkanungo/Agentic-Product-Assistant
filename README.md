...
1. **"Agent not initialized" error**:
   ...
   - Verify the Sarvam API key is valid
   - Ensure `test_catalog.json` exists and is valid

2. **"Product not found" errors**:
   - Verify product IDs in `test_catalog.json`
   - Check that the catalog file is properly formatted JSON

3. **API connection errors**:
   - Check internet connection
   - Verify Sarvam API key and endpoint
   - Check firewall settings

4. **Embedding model download issues**:
   - Ensure internet connection for first-time model download
   - The BAAI embedding model will be downloaded automatically
...
The current API uses a simple password authentication (`abcd1234`). For production use, implement proper authentication mechanisms such as:
- JWT tokens
- API keys
- OAuth 2.0
...
- **First Run**: Initial startup may take longer due to embedding model download
- **Memory Usage**: The vector index is loaded into memory for fast search
- **Concurrent Requests**: The current implementation creates a new ChatInterface for each request
...
4. Test both terminal and API interfaces
